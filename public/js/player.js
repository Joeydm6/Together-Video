...
const MOBILE_SEEK_RESUME_HOLD_MIN_MS=320;
const MOBILE_SEEK_RESUME_HOLD_MAX_MS=900;
...
const cleanAssText=text=>String(text||'').replace(/\{[^}]*\}/g,'').replace(/\\[Nn]/g,'<br>').replace(/\\h/g,' ').replace(/\s+/g,' ').replace(/(?:<br>\s*)+/g,'<br>').trim();
const parseAssOrSsa=text=>{const lines=text.replace(/\r/g,'').split('\n');let formatFields=[];const cues=[];for(const rawLine of lines){const line=rawLine.trim();if(!line)continue;if(/^Format:/i.test(line)){formatFields=line.slice(line.indexOf(':')+1).split(',').map(part=>part.trim().toLowerCase());continue;}if(!/^Dialogue:/i.test(line))continue;const payload=line.slice(line.indexOf(':')+1).trim();const parts=payload.split(',');const startIndex=formatFields.indexOf('start')>=0?formatFields.indexOf('start'):1;const endIndex=formatFields.indexOf('end')>=0?formatFields.indexOf('end'):2;const textIndex=formatFields.indexOf('text')>=0?formatFields.indexOf('text'):9;const cueText=cleanAssText(parts.slice(textIndex).join(','));if(!cueText)continue;const start=cueTime((parts[startIndex]||'').trim());const end=cueTime((parts[endIndex]||'').trim());if(!Number.isFinite(start)||!Number.isFinite(end)||end<=start)continue;cues.push({start,end,text:cueText});}return cues;};
...
const state={activeCountdownTimer:null,activeCountdownVideo:null,activeSeekId:null,activeSubtitleUrl:null,activeSyncRequestId:null,autoPlaybackModeReason:null,autoSubtitleStatus:'idle',autoplayFallbackTimer:null,availableAudioTracks:[],availableSubtitles:[],awaitingSyncSnapshot:false,bufferIssueCount:0,bufferRecoveryDone:false,capabilities:detectCapabilities(),currentUsers:0,cues:[],diagnosticEvents:[],diagnosticsTimer:null,duration:0,hasJoined:false,healthTimer:null,ignoreFullscreenExitUntil:0,inviteMenuOpen:false,isLeader:false,isPartnerReady:true,isProblematic:false,isScrubbing:false,isWatchingAlone:false,lastBroadcastReadyState:null,lastBufferIssueAt:0,lastDriftMs:null,lastHeartbeatAt:0,lastSeekAt:0,lastSnapshotRequestAt:0,latencyMs:null,leaderTimer:null,lobbyPrepared:false,lobbyPreparing:false,lobbySyncTimer:null,markedWatched:false,mediaBitrateKbps:0,mediaHeight:0,mediaPrimed:false,mediaWidth:0,nativeSubtitleTrack:null,nextAvailable:false,nextVideo:null,openingWarmupReady:false,pendingAutoplayMode:params.get('autoplay')==='true'?(params.get('transition')==='room-handoff'?'room-handoff':'direct'):null,pendingBootstrapSnapshot:null,pendingSeekResyncTimer:null,pendingSeekResume:false,parentHref:'/',playbackModeOverride:null,playerReady:false,portraitGateActive:false,postSeekSyncTimer:null,presenceLabel:'Preparing invite link...',preferDesktopFullscreen:false,prevAvailable:false,prevVideo:null,preloadedVideos:new Set(),progressTimer:null,pseudoFullscreen:false,queue:Promise.resolve(),returnToLobbyAfterFullscreenExit:false,roomCode:initialRoomCode,seekOffset:0,seekPreviewClientX:null,seekPreviewTime:null,seekResumeHoldUntil:0,seekResumeTimer:null,seekSettleUntil:0,selectedAudioTrackIndex:null,sessionStarted:false,settingsView:'home',shareUrl:initialRoomCode?`${window.location.origin}/player.html?room=${encodeURIComponent(initialRoomCode)}`:null,sourceLoadingCount:0,sourceReady:false,snapshotRetryTimer:null,sourceToken:0,startingSession:false,startupTrace:[],statusTimer:null,subtitleOffsetMs:0,syncRequestAttempts:0,uiLockedHidden:false,uiTimer:null,userMuted:false,viewingPrimed:false};
...
const hasPlaybackModePreference=()=>storageGet(localStorage,playbackModePreferenceKey)==='true';
const autoplayRequested=()=>params.get('autoplay')==='true';
const hasPendingAutoplay=()=>Boolean(state.pendingAutoplayMode);
const roomHandoffAutoplayPending=()=>state.pendingAutoplayMode==='room-handoff';
...
const shouldAutoPreferDataSaver=()=>{
if(hasPlaybackModePreference())return false;
if(currentPlaybackMode()!=='original')return false;
if(!state.capabilities.isTouch)return false;
const heavyBitrate=state.mediaBitrateKbps>=4200;
const heavyResolution=state.mediaWidth>=1920||state.mediaHeight>=1080;
const constrainedConnection=['slow-2g','2g','3g'].includes(String(state.capabilities.connection||'').toLowerCase());
if(state.capabilities.isTablet&&(state.isProblematic||heavyBitrate||heavyResolution))return true;
if(mobile()&&(state.isProblematic||constrainedConnection)&&((state.mediaBitrateKbps>=2600)||heavyResolution))return true;
return false;
};
const maybeApplyAutoPlaybackMode=()=>{
if(!shouldAutoPreferDataSaver())return false;
state.autoPlaybackModeReason=`touch-heavy-media:${state.mediaBitrateKbps||0}kbps`;
setPlaybackModeState('data-saver');
trackPlayback('auto-data-saver-default',state.autoPlaybackModeReason,{source:'capability-default',playbackMode:'data-saver'});
traceStartupStep(`playback:auto-data-saver ${state.mediaBitrateKbps||0}kbps`);
return true;
};
...
const clearSeekResumeHold=({keepPending=false}={})=>{window.clearTimeout(state.seekResumeTimer);state.seekResumeTimer=null;state.seekResumeHoldUntil=0;if(!keepPending)state.pendingSeekResume=false;};
...
const seekResumeHoldActive=()=>Date.now()<state.seekResumeHoldUntil;
const mobileSeekResumeHoldMs=()=>clamp(MOBILE_SEEK_RESUME_HOLD_MIN_MS+Math.round(Math.max(0,state.latencyMs||0)*0.6),MOBILE_SEEK_RESUME_HOLD_MIN_MS,MOBILE_SEEK_RESUME_HOLD_MAX_MS);
const shouldUseSeekResumeHold=resumeRequested=>Boolean(resumeRequested&&mobile()&&state.hasJoined&&state.currentUsers>1&&state.sessionStarted);
const finalizeSeekResume=async(reason='seek-hold')=>{
if(!state.pendingSeekResume)return false;
clearSeekResumeHold({keepPending:true});
const started=await tryPlay(false,null,{timeoutMs:iosCompat()?1400:2000,context:reason});
state.pendingSeekResume=false;
if(!started&&state.hasJoined&&state.currentUsers>1&&!state.isLeader&&!seekWindowActive()){
requestLiveSync('seek-resume-hold',{silent:true,force:true});
}
return started;
};
const enterSeekResumeHold=(resumeRequested,{reason='seek'}={})=>{
if(!shouldUseSeekResumeHold(resumeRequested))return false;
const holdMs=mobileSeekResumeHoldMs();
state.pendingSeekResume=true;
state.seekResumeHoldUntil=Date.now()+holdMs;
window.clearTimeout(state.seekResumeTimer);
state.seekResumeTimer=window.setTimeout(()=>{
state.seekResumeTimer=null;
if(!state.pendingSeekResume||seekResumeHoldActive())return;
void finalizeSeekResume(`${reason}-timeout`);
},holdMs+120);
if(!els.video.paused)els.video.pause();
traceStartupStep(`seek:hold ${reason} ${holdMs}ms`);
return true;
};
...
socket.emit('syncSnapshot',{seekId,time:currentTime(),paused:state.pendingSeekResume?false:els.video.paused,subtitleUrl:state.activeSubtitleUrl,subtitleOffsetMs:state.subtitleOffsetMs,audioTrackIndex:state.selectedAudioTrackIndex});
traceStartupStep(`seek:stabilize ${reason}`);
if(state.pendingSeekResume)window.setTimeout(()=>{if(state.pendingSeekResume&&!seekResumeHoldActive())void finalizeSeekResume(`leader-seek-settle-${reason}`);},110);
},420);
};
...
const clearAutoplayFallback=()=>{clearTimeout(state.autoplayFallbackTimer);state.autoplayFallbackTimer=null;};
const syncHistoryParams=()=>{const clean=params.toString();window.history.replaceState({},document.title,clean?`${window.location.pathname}?${clean}`:window.location.pathname);};
const consumePendingAutoplay=()=>{clearAutoplayFallback();state.pendingAutoplayMode=null;params.delete('autoplay');params.delete('transition');syncHistoryParams();};
const scheduleAutoplayFallback=()=>{
if(!roomHandoffAutoplayPending()||state.autoplayFallbackTimer||!state.hasJoined||!state.isLeader||state.currentUsers>1)return;
state.autoplayFallbackTimer=window.setTimeout(()=>{
if(!roomHandoffAutoplayPending()||!state.hasJoined||!state.isLeader||state.currentUsers>1)return;
...
const maybeStartPendingAutoplay=reason=>{
if(!hasPendingAutoplay()||!state.hasJoined)return false;
if(!roomHandoffAutoplayPending()){
requestPlay();
consumePendingAutoplay();
return true;
}
if(state.currentUsers<=1){
scheduleAutoplayFallback();
renderJoinOverlay();
return false;
}
clearAutoplayFallback();
if(!state.isLeader||!canStartFromLobby()){
state.startingSession=true;
renderJoinOverlay();
return false;
}
traceStartupStep(`autoplay:handoff ${reason}`);
requestPlay();
consumePendingAutoplay();
return true;
};
...
const applyMediaCapabilityDefaults=()=>{const changed=maybeApplyAutoPlaybackMode();syncPlaybackModeUi();return changed;};
...
const goToVideo=(targetVideo,{autoplay=true,transition=null}={})=>{if(!targetVideo)return;const next=new URL('/player.html',window.location.origin);next.searchParams.set('video',targetVideo);if(state.roomCode)next.searchParams.set('room',state.roomCode);if(autoplay)next.searchParams.set('autoplay','true');if(transition)next.searchParams.set('transition',transition);try{cleanup();}catch(error){console.debug('[NAVIGATE_CLEANUP]',error?.message||error);}if(socket.connected){try{socket.disconnect();}catch(error){console.debug('[NAVIGATE_SOCKET]',error?.message||error);}}window.location.href=`${next.pathname}?${next.searchParams.toString()}`;};
...
const seekTo=async(target,broadcast=true)=>{const total=duration();const boundedMax=total>0?total:Number.POSITIVE_INFINITY;const safe=Math.max(0,Math.min(boundedMax,Number(target)||0));const seekId=broadcast&&state.hasJoined?createSeekId():null;const shouldResume=!els.video.paused;const holdSeekResume=enterSeekResumeHold(shouldResume,{reason:'local-seek'});registerSeekWindow(seekId,{allowFallback:Boolean(seekId&&state.hasJoined&&!state.isLeader&&state.currentUsers>1)});recordPlaybackEvent('seek-commit',`${Math.round(safe)}s`,{seekId,source:'local-seek',playbackMode:currentPlaybackMode(),driftMs:0});if(!directPlaybackMode())await loadSource({time:safe,resume:shouldResume&&!holdSeekResume,reason:'seek'});else{state.seekOffset=0;els.video.currentTime=safe;syncUi(safe);}if(broadcast&&state.hasJoined)socket.emit('seek',{seekId,time:safe,timestamp:Date.now()});if(state.hasJoined&&state.currentUsers>1&&state.isLeader)scheduleLeaderSeekSync('local',seekId);};
...
const loadSubtitleFile=async url=>{if(!url){state.cues=[];els.subtitleContainer.style.visibility='hidden';syncNativeSubtitleTrack();return;}const response=await fetch(url);if(!response.ok)throw new Error(`Failed to load subtitle: ${response.status}`);const text=await response.text();const normalizedUrl=String(url).split('?')[0].toLowerCase();if(text.startsWith('WEBVTT')||normalizedUrl.endsWith('.vtt'))state.cues=parseVtt(text);else if(normalizedUrl.endsWith('.ass')||normalizedUrl.endsWith('.ssa'))state.cues=parseAssOrSsa(text);else state.cues=parseSrt(text);syncNativeSubtitleTrack();};
const setSubtitle=async(url,{broadcast=false,silent=false}={})=>{const previous=state.activeSubtitleUrl;state.activeSubtitleUrl=url;storeSubtitleSelection(url||'off');renderSubtitleButtons();try{await loadSubtitleFile(url);renderSubtitle();if(broadcast)broadcastSubtitleSelection(url);}catch(error){state.activeSubtitleUrl=previous;storeSubtitleSelection(previous||'off');renderSubtitleButtons();throw error;}if(!silent)status(url?'Subtitle updated':'Subtitles turned off',1500);};
...
const fetchVideoInfo=async()=>{traceStartupStep('video info:request');const response=await fetch(`/api/video-info?video=${encodeURIComponent(videoFile)}`);if(!response.ok)throw new Error(`Failed to load video info (${response.status})`);const data=await response.json();state.duration=Number(data.duration)||0;state.isProblematic=Boolean(data.isProblematic);state.mediaBitrateKbps=Math.max(0,Number(data.bitrateKbps)||0);state.mediaWidth=Math.max(0,Number(data.width)||0);state.mediaHeight=Math.max(0,Number(data.height)||0);state.availableAudioTracks=Array.isArray(data.audioTracks)?data.audioTracks:[];state.selectedAudioTrackIndex=defaultAudioTrackIndex();renderAudioTracks();applyMediaCapabilityDefaults();syncUi(0);traceStartupStep('video info:ready');};
...
socket.on('seek',data=>{enqueue(async()=>{state.latencyMs=data?.serverTimestamp?Math.max(0,Date.now()-data.serverTimestamp):state.latencyMs;const shouldResume=!els.video.paused&&state.sessionStarted;const holdSeekResume=enterSeekResumeHold(shouldResume,{reason:'remote-seek'});registerSeekWindow(data?.seekId||null);const driftMs=await syncTo(data?.time,{threshold:SEEK_DRIFT,resume:shouldResume&&!holdSeekResume,reason:'remote-seek'});recordPlaybackEvent('seek',`${Math.round(Number(data?.time)||0)}s`,{seekId:data?.seekId||null,source:'remote-seek',playbackMode:currentPlaybackMode(),driftMs});if(shouldResume&&!holdSeekResume){const resumed=await tryPlay(false,null,{timeoutMs:iosCompat()?1200:1800,context:'remote-seek'});if(!resumed&&state.hasJoined&&state.currentUsers>1&&!state.isLeader&&!seekWindowActive()){requestLiveSync('seek-resume',{silent:true,force:true});}}if(state.isLeader)scheduleLeaderSeekSync('remote',data?.seekId||null);status('Position synchronized',1200);updateSyncStatus();});});
...
socket.on('syncSnapshot',data=>{enqueue(async()=>{if(data?.videoFile&&data.videoFile!==videoFile)return;if(!matchesActiveSyncRequest(data))return;traceStartupStep(`socket:snapshot ${Math.round(Number(data?.time)||0)}s`);clearAutoplayFallback();const settleSnapshot=matchingSeekSnapshot(data);if(settleSnapshot)clearPendingSeekResync();state.latencyMs=data?.serverTimestamp?Math.max(0,Date.now()-data.serverTimestamp):state.latencyMs;if(data?.audioTrackIndex!==undefined&&data.audioTrackIndex!==null)await setAudioTrackSelection(data.audioTrackIndex,{broadcast:false,silent:true});const driftMs=await syncTo(data?.time,{threshold:settleSnapshot?SNAPSHOT_SEEK_SETTLE_DRIFT:0.05,resume:!(data?.paused)&&!state.pendingSeekResume,reason:settleSnapshot?'seek-settle-snapshot':'snapshot'});recordPlaybackEvent('snapshot',`${Math.round(Number(data?.time)||0)}s`,{seekId:data?.seekId||null,source:settleSnapshot?'seek-settle-snapshot':'snapshot',playbackMode:currentPlaybackMode(),driftMs});if(data?.paused){clearSeekResumeHold();els.video.pause();}else{if(mobile()){const ready=await requestLandscapeViewing({reusePrimed:true});if(!ready){traceStartupStep('socket:snapshot-blocked');state.sessionStarted=false;showJoinOverlay();updatePortraitGate();completeSyncRequest();updateSyncStatus();return;}}else await ensureDesktopFullscreen();let started=false;if(state.pendingSeekResume&&settleSnapshot)started=await finalizeSeekResume('seek-settle-snapshot');else started=await tryPlay(true,null,{timeoutMs:iosCompat()?1400:2200,context:'sync-snapshot'});if(!started){traceStartupStep('socket:snapshot-timeout');state.sessionStarted=false;showJoinOverlay();updatePortraitGate();completeSyncRequest();updateSyncStatus();return;}state.sessionStarted=true;hideJoinOverlay({immediate:true});}setSubtitleOffset(data?.subtitleOffsetMs??state.subtitleOffsetMs,{broadcast:false,notify:false});if(data?.subtitleUrl!==undefined){try{await setSubtitle(data.subtitleUrl||null,{silent:true});}catch(error){console.error('[SYNC SNAPSHOT]',error);}}completeSyncRequest();updateSyncStatus();if(!settleSnapshot)status('Resynced with partner',1500);});});
socket.on('playRequestTimedOut',()=>{state.startingSession=false;clearAutoplayFallback();completeSyncRequest();renderJoinOverlay();status('Partner never became ready. Playback stayed paused to keep sync intact.',3500);});
...
socket.on('loadNextVideo',({video})=>{clearNextEpisodeCountdown();status('Loading next episode...',1500);window.setTimeout(()=>{goToVideo(video,{autoplay:true,transition:state.hasJoined&&state.currentUsers>1?'room-handoff':null});},900);});
socket.on('loadPreviousVideo',({video})=>{clearNextEpisodeCountdown();status('Loading previous episode...',1500);window.setTimeout(()=>{goToVideo(video,{autoplay:true,transition:state.hasJoined&&state.currentUsers>1?'room-handoff':null});},900);});
...
els.video.addEventListener('play',()=>{clearSeekResumeHold();restoreAudiblePlayback();updatePlayPause();updateLeaderSync();syncMediaSessionPlaybackState();syncMediaSessionPosition();if(!state.uiLockedHidden)showUi();});
...
els.video.addEventListener('pause',()=>{if(!state.pendingSeekResume)clearSeekResumeHold();updatePlayPause();updateLeaderSync();syncMediaSessionPlaybackState();syncMediaSessionPosition();showUi();clearTimeout(state.uiTimer);});
