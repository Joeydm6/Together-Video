...
const goToVideo=(targetVideo,{autoplay=true,transition=null}={})=>{if(!targetVideo)return;const next=new URL('/player.html',window.location.origin);next.searchParams.set('video',targetVideo);if(state.roomCode)next.searchParams.set('room',state.roomCode);if(autoplay)next.searchParams.set('autoplay','true');if(transition)next.searchParams.set('transition',transition);window.location.href=`${next.pathname}?${next.searchParams.toString()}`;};
