...
const seekTo=async(target,broadcast=true)=>{const total=duration();const boundedMax=total>0?total:Number.POSITIVE_INFINITY;const safe=Math.max(0,Math.min(boundedMax,Number(target)||0));const seekId=broadcast&&state.hasJoined?createSeekId():null;registerSeekWindow(seekId,{allowFallback:Boolean(seekId&&state.hasJoined&&!state.isLeader&&state.currentUsers>1)});if(!directPlaybackMode())await loadSource({time:safe,resume:!els.video.paused,reason:'seek'});else{state.seekOffset=0;els.video.currentTime=safe;syncUi(safe);}if(broadcast&&state.hasJoined)socket.emit('seek',{seekId,time:safe,timestamp:Date.now()});if(state.hasJoined&&state.currentUsers>1&&state.isLeader)scheduleLeaderSeekSync('local',seekId);};
...
socket.on('roomSyncBootstrap',data=>{enqueue(async()=>{if(data?.videoFile&&data.videoFile!==videoFile)return;const deferBootstrap=Boolean(!state.sessionStarted&&state.currentUsers>1&&!state.isLeader&&(!state.lobbyPrepared||!openingSecondsReady()));if(deferBootstrap){state.pendingBootstrapSnapshot=data;recordPlaybackEvent('bootstrap-deferred',`${Math.round(Number(data?.time)||0)}s`);updateSyncStatus();renderJoinOverlay();return;}await applyBootstrapSnapshot(data);});});
...
socket.on('seek',data=>{enqueue(async()=>{recordPlaybackEvent('seek',`${Math.round(Number(data?.time)||0)}s`);state.latencyMs=data?.serverTimestamp?Math.max(0,Date.now()-data.serverTimestamp):state.latencyMs;registerSeekWindow(data?.seekId||null);await syncTo(data?.time,{threshold:SEEK_DRIFT,resume:!els.video.paused,reason:'remote-seek'});if(!els.video.paused&&state.sessionStarted){const resumed=await tryPlay(false,null,{timeoutMs:iosCompat()?1200:1800,context:'remote-seek'});if(!resumed&&state.hasJoined&&state.currentUsers>1&&!state.isLeader&&!seekWindowActive()){requestLiveSync('seek-resume',{silent:true,force:true});}}if(state.isLeader)scheduleLeaderSeekSync('remote',data?.seekId||null);status('Position synchronized',1200);updateSyncStatus();});});
...
els.video.addEventListener('waiting',()=>{traceStartupStep(`media:waiting ${bufferedAhead().toFixed(2)}s`);setReady(false,'waiting');const withinPostSeekCooldown=seekWindowActive()&&(Date.now()-state.lastSeekAt)<SEEK_BUFFER_SYNC_COOLDOWN_MS;if(state.sessionStarted&&(Date.now()-state.lastSeekAt)<2500&&!withinPostSeekCooldown){if(state.isLeader)scheduleLeaderSeekSync('waiting',state.activeSeekId);else requestLiveSync('seek-waiting',{silent:true,force:true});}maybeRecoverFromStartupBuffer('waiting');});
els.video.addEventListener('stalled',()=>{traceStartupStep(`media:stalled ${bufferedAhead().toFixed(2)}s`);setReady(false,'stalled');const withinPostSeekCooldown=seekWindowActive()&&(Date.now()-state.lastSeekAt)<SEEK_BUFFER_SYNC_COOLDOWN_MS;if(state.sessionStarted&&(Date.now()-state.lastSeekAt)<2500&&!withinPostSeekCooldown){if(state.isLeader)scheduleLeaderSeekSync('stalled',state.activeSeekId);else requestLiveSync('seek-stalled',{silent:true,force:true});}maybeRecoverFromStartupBuffer('stalled');});
